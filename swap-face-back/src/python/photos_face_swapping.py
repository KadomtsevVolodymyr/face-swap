...
def swap_face(img1, img2):
    ...
    resized_samlessclone = cv2.resize(seamlessclone, (600, 800))
    # cv2.imshow("seamlessclone", resized_samlessclone)
    # cv2.waitKey(0)
    timestamp = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
    output_filename = "swapped_faces"+ timestamp +".jpg"
    output_path = "dist/swapped_face/" + output_filename



    cv2.imwrite(output_path, resized_samlessclone)
    return output_path
